 ...
 //v5
  ...
  toggleButton.onclick = function () {
    if (!uiLoaded) {
      ...
      const root = document.getElementById("atemp-root");
      if (root) {
  ...
  `;
    ...
    function updateCSSOut() {
      ...
      const styleMap = {};
      if (cssMap[selector]) {
        const existingRules = cssMap[selector].match(/{([^}]+)}/)?.[1] || '';
        existingRules.split(';').forEach(rule => {
          const [prop, value] = rule.split(':').map(s => s.trim());
          if (prop && value) styleMap[prop] = value;
        });
      }
    ...
    function updateCssOutBox() {
      ...
      const allCSS = Object.entries(cssMap)
        .map(([selector, rules]) => rules)
        .join("\n\n");
      box.value = allCSS;
    }
...
`;
  ...
  `;
        ...
        content.querySelector("#atemp-genjs")
          .onclick = () => {
            ...
            if (!css.trim()) return;
            const js =
              `(() => {const style=document.createElement('style');style.textContent=\`${css.replace(/`/g,"\\`")}\`;document.head.appendChild(style);})();`;
  ...
  `;
      ...
      if (!selected) {
        html =
          `<div class="atemp-tip">Hold <b>Shift</b> and hover any element to select and edit its style live.</div>`;
      } else {
    ...
    <div class="atemp-selrow">
      ...
      <button id="atemp-copyselector" class="atemp-btn" style="width:auto;padding:2px 7px;">Copy</button>
        ...
        .forEach(field => {
          let k = field.k,
            v = selected.style[k] || "";
          let type = field.type || (k.includes("color") ? "color" : (k === "opacity" || k === "zIndex" || k ===
      ...
      if (selected) {
        ...
        (tabConfig[tab]?.fields || []).forEach(field => {
          const k = field.k;
          const type = field.type || (k.includes("color") ? "color" : (k === "opacity" || k === "zIndex" || k ===
            "fontWeight") ? "number" : "text");
          if (type === "color") {
    ...
    document.addEventListener("mousemove", e => {
      ...
      underCursor = document.elementsFromPoint(e.clientX, e.clientY)
        .filter(
          el => !root.contains(el) && el.id !== "atemp-overlay" && !el.classList.contains("atemp-tooltip") && el
          .id !== "atemp-preview-tooltip"
        );
